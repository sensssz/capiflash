...
long diff_time(struct timeval start, struct timeval end) {
...
int main(int argc, char **argv)
{
    ...
    for (nblocks = 1; nblocks <= 4096; nblocks += 128) {
        ...
        if (nRD > 0) {
            ...
            while (N)
            {
                ...
                if      (0 == rc) {--RD; lba+=nblocks; if (lba >= nblks) lba=cnt%2; ++ops;}
            ...
            while (N)
            {
                ...
                rc = cblk_awrite(id, wbuf[tag], lba, nblocks, &tag, NULL,
                                 CBLK_ARW_WAIT_CMD_FLAGS | CBLK_ARW_USER_TAG_FLAG);
                if      (0 == rc) {--WR; lba+=nblocks; if (lba >= nblks) lba=cnt%2; ++ops;}
                else if (EBUSY == errno) {++N; usleep(USLEEP); continue;}
                else                     {io_error(id,errno);}
            }
        }
        ...
        long total_time = diff_time(op_start, op_end);
